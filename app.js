...
// display every item in the array (more than 100 items)
...
game.party.forEach((poke, index) => {
  ...
  game.party.splice(index,1, pokemon[1])
  }
  else if(poke.name === 'Charmander'){
    game.party.splice(index,1, pokemon[4])
  }
  else if(poke.name === 'Squirtle'){
    game.party.splice(index,1, pokemon[7])
  }
  else if(poke.name === 'Pikachu'){
    game.party.splice(index,1, pokemon[25])
  }
...
console.log('Exercise 9result:', starterPokemon)
...
function partyCount(game) {
  return game.party.length
}



console.log('Exercise 14 result:',partyCount(game))
